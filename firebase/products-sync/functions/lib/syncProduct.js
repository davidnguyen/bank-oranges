      ...
      while (page <= totalPages) {
        ...
        if (products.length && products.length > 0) {
          await Promise.all(products.map(async (p) => {
            ...
            const aggregatorQuerySnapshot = await db.collection("aggregators")
              .where("source", "==", "products").get();
            const aggregators = aggregatorQuerySnapshot.docs
              .map((s) => s.data())
              .reduce((prev, current) => ({...prev, [current.name]: 0}), {});
